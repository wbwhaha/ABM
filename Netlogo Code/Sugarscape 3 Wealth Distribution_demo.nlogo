globals [
  ...
  dead-due-to-poverty
  dead-due-to-aging
  prevalence-of-poverty-death
  prevalence-of-aging-death
]
...
to setup
  ...
  clear-all
  set death 0
  set dead-due-to-poverty 0
  set dead-due-to-aging 0
  set prevalence-of-poverty-death 0
  set prevalence-of-aging-death 0
...
to turtle-setup ;; turtle procedure
  ...
  set metabolism random-in-range 1 max-metabolism
  set max-age random-in-range min-death-age 100
  set age 0
...
to go
  ...
  ask turtles [
    ...
    turtle-eat
    set age (age + 1)
    if (sugar <= 0 and age > max-age) [
      set death death + 1
      set dead-due-to-poverty dead-due-to-poverty + 1
      set dead-due-to-aging dead-due-to-aging + 1
  ...
  if dead-due-to-poverty > 0 [
    set prevalence-of-poverty-death (dead-due-to-poverty / death)]
  if dead-due-to-aging > 0 [
    set prevalence-of-aging-death (dead-due-to-aging / death)]

  ;print percentage-of-child-poverty
  ;print percentage-of-sugar-controlled-by-rich
...
to-report percentage-of-child-poverty

  ; Sort turtles by sugar (ascending order)
  let sorted-turtles sort-on [sugar] turtles

  ; Calculate the number of poor turtles (bottom 20%)
  let poor-count max (list 1 (floor (0.2 * length sorted-turtles)))  ;; Ensure at least 1 turtle

  ; Get the poorest 20% of turtles
  let poor-turtles sublist sorted-turtles 0 poor-count

  ; Count children in the poor group
  let children-count count (turtle-set poor-turtles) with [age < 18]

  ; Calculate proportion of children in the poor group (prevent division by zero)
  report ifelse-value (poor-count > 0) [children-count / poor-count] [0]

end

to-report percentage-of-sugar-controlled-by-rich

  ; Sort turtles by sugar (descending order) so richest come first
  let sorted-turtles reverse sort-on [sugar] turtles

  ; Determine number of rich turtles (top 20%)
  let total-turtles count turtles
  let rich-count max (list 1 (floor (0.2 * total-turtles)))

  ; Get top 20% richest turtles
  let rich-turtles sublist sorted-turtles 0 rich-count

  ; Calculate total sugar in the entire population
  let total-sugar sum [sugar] of turtles

  ; Calculate sugar controlled by the richest turtles
  let rich-sugar sum [sugar] of turtle-set rich-turtles

  ; Compute percentage (prevent division by zero)
  report ifelse-value (total-sugar > 0) [rich-sugar / total-sugar] [0]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot prevalence-of-poverty-death"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot prevalence-of-aging-death"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot percentage-of-child-poverty"
...
1355
595
Percentage of sugar controlled by rich
NIL
NIL
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot percentage-of-sugar-controlled-by-rich"
...
@#$#@#$#@
<experiments>
  <experiment name="Final_s3" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <metric>prevalence-of-poverty-death</metric>
    <metric>prevalence-of-aging-death</metric>
    <metric>count turtles / initial-population</metric>
    <metric>gini-index</metric>
    <metric>percentage-of-child-poverty</metric>
    <metric>percentage-of-sugar-controlled-by-rich</metric>
    <enumeratedValueSet variable="growback-rate">
      <value value="1"/>
      <value value="2"/>
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="min-death-age">
      <value value="1"/>
      <value value="60"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="max-metabolism">
      <value value="2"/>
      <value value="4"/>
      <value value="10"/>
    </enumeratedValueSet>
